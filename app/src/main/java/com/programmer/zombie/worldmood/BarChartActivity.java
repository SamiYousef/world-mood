...
public class BarChartActivity extends AppCompatActivity {
    ...
    ArrayList<BarEntry> valueSetPos, valueSetNegative;
    ArrayList<String> labels;

    Handler handler;
    Runnable runnable;

    BarDataSet negativeLine, positiveLine;
    int i = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        negativeLine = new BarDataSet(valueSetNegative, "Negative Line");
        negativeLine.setColor(Color.RED);

        positiveLine = new BarDataSet(valueSetPos, "Positive Line");
            ...
            public void onSuccess(String response) {
                try {
                    ...
                    if (obj.getBoolean("status")) {
                        ...
                        positiveLine.addEntry(new BarEntry(pos, i));
                        negativeLine.addEntry(new BarEntry(neg, i));
                        labels.add(time);
                        //update line chart

                        BarChart chart = (BarChart) findViewById(R.id.barChart);

                        BarData data = new BarData(labels, positiveLine);
                        data.addDataSet(negativeLine);
                        chart.setData(data);
                        chart.setDescription(MainActivity.tweet + "  Bar Chart");
